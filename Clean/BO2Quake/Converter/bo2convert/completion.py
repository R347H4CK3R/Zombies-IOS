...
REQUIRED_FAMILIES = (
    'worlds', 'collision', 'materials', 'textures', 'models', 'animations',
    'audio', 'weapons', 'attachments', 'effects', 'entities', 'scripts',
    'gamemodes', 'ai', 'vehicles', 'ui', 'localization', 'cinematics',
)
...
def validate_completion(gamedata_root: Path | str) -> CompletionResult:
    ...
    if not isinstance(assets, list):
        failures.append('manifest assets must be an array'); assets = []
    ids: set[str] = set()
    for index, asset in enumerate(assets):
        if not isinstance(asset, dict):
            failures.append(f'asset {index} is not an object'); continue
        asset_id = str(asset.get('id', '')); kind = str(asset.get('kind', ''))
        if not asset_id: failures.append(f'asset {index} has no id')
        elif asset_id in ids: failures.append(f'duplicate asset id: {asset_id}')
        ids.add(asset_id)
        if kind in counts: counts[kind] += 1
        if asset.get('validation') != 'complete': failures.append(f'asset not complete: {asset_id or index}')
        payloads = asset.get('payloads') or []
        if not payloads: failures.append(f'asset has no payloads: {asset_id or index}')
        for raw in payloads:
            try: payload = _safe_payload(root, str(raw))
            except ValueError as exc: failures.append(str(exc)); continue
            if not payload.is_file(): failures.append(f'missing payload: {raw}'); continue
            if payload.suffix.lower() in RETAIL_EXTENSIONS:
                failures.append(f'raw retail container not allowed in GameData: {raw}')
    for family, count in counts.items():
        if count == 0: failures.append(f'missing required asset family: {family}')
    dependency_ids = {str(a.get('id')) for a in assets if isinstance(a, dict) and a.get('id')}
    for asset in assets:
