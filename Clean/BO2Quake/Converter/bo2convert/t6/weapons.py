...
def _optional_int(values: dict[str, str], names: tuple[str, ...], default: int) -> int:
    for name in names:
        ...
        if raw not in (None, ''):
            ...
            except ValueError as exc: raise ValueError(f'invalid integer weapon field {name}={raw!r}') from exc
    return default
...
def export_weapon(info_text: str, destination: Path | str, *, weapon_id: str) -> WeaponExportResult:
    ...
    if clip_size <= 0 or max_ammo < clip_size or fire_ms <= 0 or reload_ms <= 0 or damage < 0:
    ...
    data = {
        ...
        'damage': damage,
        'minDamage': _optional_int(values, ('minDamage','iMinDamage'), damage),
